...
/**
 * Gets all information about a single student
 *
...
/**
 * Gets a single rubric's information. 
 *
...
/**
 * Gets a rubric number from a quality. Usefull for checking if a teacher owns
 * a rubric based on the supplied quality.
 *
 * $rubricQualityNum: A rubric quality's number in the database
 * return: A single rubric.
 */
 ...
 */
function sql_getAllStudentClasses($teacherNum, $studentNum) {
	global $conn;
	$stmt = $conn->prepare(
<<<SQL
 ...
 */
function sql_getAllClasses($teacherNum) {
	global $conn;
	$stmt = $conn->prepare(
<<<SQL
 ...
 */
function sql_getAllStudentsInClass($classNum) {
	global $conn;
	$stmt = $conn->prepare(
<<<SQL
 ...
 * $teacherNum: The number of the teacher in the database.
 */
function sql_getAllRubrics($teacherNum) {
	global $conn;
	$stmt = $conn->prepare(
<<<SQL
...
function sql_getAllRubricsBasedOnSearch($teacherNum, $searchTerm) {
	global $conn;
	$stmt = $conn->prepare(
<<<SQL
SELECT NUM, TEACHER_NUM, MAX_POINTS_PER_CRITERIA, SUBTITLE, ((
 ...
 */
function sql_getAllCriteriaComponents($criteriaNum) {
	global $conn;
	$stmt = $conn->prepare(
<<<SQL
...
function sql_getAllRubricAssignments($teacherNum, $rubricNum) {
	global $conn;
	$stmt = $conn->prepare(
<<<SQL
...
function sql_unbindRubricFromAssignment($rubricNum, $assignmentNum) {
	global $conn;
	$stmt = $conn->prepare(
<<<SQL
DELETE FROM `ASSIGNMENT-RUBRIC_LINKER` 
WHERE RUBRIC_NUM = :rubricnum AND 
